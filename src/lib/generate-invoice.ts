...
const PRIMARY = rgb(37 / 255, 99 / 255, 235 / 255); // #2563eb
...
export function generateInvoicePdf(data: InvoiceData): Promise<Uint8Array> {
  return _generate(data);
}

async function _generate(data: InvoiceData): Promise<Uint8Array> {
  ...
  // Company info
  y -= 15;
  page.drawText(COMPANY.legalName, { x: MARGIN, y, size: 14, font: fontBold, color: PRIMARY });
  y -= 16;
  ...
  // Customer info box
  const clientLines = [data.customerName, data.customerEmail];
  ...
  let clientY = y - 20;
  page.drawText(data.customerName, { x: MARGIN + 12, y: clientY, size: 11, font: fontBold, color: TEXT });
  clientY -= 14;
  ...
  page.drawText(`Reference : ${data.stripeSessionId}`, {
